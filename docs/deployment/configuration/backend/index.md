...
| `IDP_CLAIMS_ROLES_PATH` | [Keycloak API client](/deployment/configuration/keycloak/#client-api) roles path. | Yes | - | `<keycloak_client>.roles` |
| `SECURITY_AUDIENCE` | [Keycloak API client scope](/deployment/configuration/keycloak/#step-4-create-client-scopes) that security is intended for. | Yes | - | `<keycloak-client-scope>` |
...
| Env Variable | Description | Required | Default Value | Example |
| --- | --- | --- | --- | --- |
| `FILE_TRANSFORMER_AUTH_TOKEN_ENDPOINT` | URL to get keycloak auth token for [file transformer services](/docs/optional-services/file-transformers/index.md). | Yes** | - | `http://<keycloak-hostname>:<port>/realms/<realm-name>/protocol/openid-connect/token` |
...
| Env Variable | Description | Required | Default Value | Example |
| --- | --- | --- | --- | --- |
| `DEPOSIT_AUTH_TOKEN_ENDPOINT` | URL to get keycloak auth token for [deposit services](/docs/optional-services/deposit-services/index.md). | Yes*** | - | `http://<keycloak-hostname>:<port>/realms/<realm-name>/protocol/openid-connect/token` |
...
| Env Variable | Description | Required | Default Value | Example |
| --- | --- | --- | --- | --- |
| `EVALUATOR_AUTH_TOKEN_ENDPOINT` | URL to get keycloak auth token for [evaluator services](/optional-services/evaluator-services.md). | Yes**** | - | `http://<keycloak-hostname>:<port>/realms/<realm-name>/protocol/openid-connect/token` |
...
| Env Variable | Description | Required | Default Value | Example |
| --- | --- | --- | --- | --- |
| `SUPPORTIVE_MATERIAL_USER_GUIDE_PATH` | File path to store Supportive Material [User Guide](/administering/system-configuration/supportive-material.md). | No | `classpath:/material/using/` | `classpath:<file-path>` |
| `SUPPORTIVE_MATERIAL_ABOUT_PATH` | File path to store Supportive Material [About](/administering/system-configuration/supportive-material.md). | No | `classpath:/material/about/` | `classpath:<file-path>` |
| `SUPPORTIVE_MATERIAL_TERM_OF_SERVICE_PATH` | File path to store Supportive Material [Terms of Service](/administering/system-configuration/supportive-material.md). | No | `classpath:/material/terms-of-service/` | `classpath:<file-path>` |
| `SUPPORTIVE_MATERIAL_GLOSSARY_PATH` | File path to store Supportive Material [Glossary](/administering/system-configuration/supportive-material.md). | No | `classpath:/material/glossary/` | `classpath:<file-path>` |
| `SUPPORTIVE_MATERIAL_FAQ_PATH` | File path to store Supportive Material [FAQ](/administering/system-configuration/supportive-material.md). | No | `classpath:/material/faq/` | `classpath:<file-path>` |
| `SUPPORTIVE_MATERIAL_COOKIE_POLICY_PATH` | File path to store Supportive Material [Cookie Policy](/administering/system-configuration/supportive-material.md). | No | `classpath:/material/cookie-policy/` | `classpath:<file-path>` |
| `SUPPORTIVE_MATERIAL_LANGUAGE_PATH` | File path to store [Language](/administering/system-configuration/languages.md) files. | No | `file:frontend/src/assets/i18n/` |`file:<file-path>` |
...
| Env Variable | Description | Required | Default Value | Example |
| --- | --- | --- | --- | --- |
| `TENANT_CONFIG_ENCRYPTION_AES_KEY` | [AES key](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard) used to encrypt keycloak client secret for [file transformer](/docs/optional-services/file-transformers/index.md), [deposit](/docs/optional-services/deposit-services/index.md), [evaluator](/optional-services/evaluator-services.md) services. | No | `rmpTvZnRWzyisUtFADBcZCn0q7Z75Xdz` | `rmpTvZnRWzyisUtFADBcZCn0q7Z75Xdz` |
| `TENANT_CONFIG_ENCRYPTION_AES_IV` | [AES IV](https://en.wikipedia.org/wiki/Initialization_vector) used to encrypt keycloak client secret for [file transformer](/docs/optional-services/file-transformers/index.md), [deposit](/docs/optional-services/deposit-services/index.md), [evaluator](/optional-services/evaluator-services.md) services. | No | `ec05d521a23f80ad` | `ec05d521a23f80ad` |

#### Notification Expiration Settings

| Env Variable | Description | Required | Default Value | Example |
| --- | --- | --- | --- | --- |
| `MERGE_ACCOUNT_EXPIRATION` | Expiration time (in seconds) for [link account notification](/using/profile-settings.md#link-account). | No | `14400` | `14400` |
| `REMOVE_CREDENTIAL_EXPIRATION` | Expiration time (in seconds) for [unlink account notification](/using/profile-settings.md#unlink-account). | No | `14400` | `14400` |
| `TENANT_SPECIFIC_INVITATION_EXPIRATION` | Expiration time (in seconds) for [user invite notification to tenant](/administering/user-management/tenant-users.md). | No | `2592000` | `2592000` |
| `PLAN_INVITATION_EXTERNAL_USER_EXPIRATION` | Expiration time (in seconds) for [notification of invitation of external users to a plan](/using/plans/invite-collaborators.md). | No | `2592000` | `2592000` |
...
| Env Variable | Description | Required | Default Value | Example |
| --- | --- | --- | --- | --- |
| `ACCOUNTING_ENABLED` | Boolean value to enable [accounting service](/optional-services/accounting-services/accounting-preview.md). | No | `false` | `true` |
...
| Env Variable | Description | Required | Default Value | Example |
| --- | --- | --- | --- | --- |
| `KPI_VIEWER_TASK_ENABLED` | Boolean value to enable [KPI service](/optional-services/kpi.md). | No | `false` | `true` |
...
| Env Variable | Description | Required | Default Value | Example |
| --- | --- | --- | --- | --- |
| `SWAGGER_AUTH_CLIENT_ID` | Keycloak [Swagger](/deployment/configuration/keycloak/#client-swagger) client id | Yes | - | `<keycloak-client-id>` |
| `SWAGGER_AUTH_REDIRECT_URL` | Swagger redirect URL (must be the same value with [swagger keycloak client](/deployment/configuration/keycloak/#client-swagger)). | Yes | - | `https://<app-hostname>/api/swagger-ui/oauth2-redirect.html` |
| `SWAGGER_SERVER_URL` | Server URL. | Yes | - | `https://<app-hostname>/api` |
...
- [Keycloak Configuration](/deployment/configuration/keycloak/) - Authentication and authorization setup
