...
# Deposit Services
There are currently five implemented [deposit services](/docs/optional-services/deposit-services/index.md). Here you can view all available configuration options for each deposit service.
...
3. **User Profile Settings** — Users store their personal Zenodo access token in their [OpenCDMP profile settings](/using/profile-settings.md). The token is retrieved per-user at deposit time and remains valid until the user updates it.
...
- [Deposit Service Authentication](/deployment/configuration/backend/index.md#deposit-service-authentication) - Backend authentication setup for deposit services
