...
The user can view the descriptions he has created or is a contributor. The basic information of the descriptions is displayed in a card format. At the top of the cards, the `name` and the `date` of creation or update are displayed. At the main body of the cards, the user can view the `status` of the description (more information [here](administering/system-configuration/description-statuses.md)), the `version`, the `grant` that is related with the description, if any, and the role he has on it. The [plan](using/plans/index.md) is also listed. At the bottom of the cards, the user has access to some of the actions that he can perform on the description, based on his plan specific role (*all the actions exist in [description overview page](using/descriptions/description-overview.md)*).
...
- **Export**: With this option, a dropdown containing all the export options pops up. You can view more details on the exports [here](using/descriptions/exports.md).<br/>*Every collaborator can do this.*
...
- **Related Plans**: Filter descriptions based on the the [tenant](administering/tenant-management/tenants.md) your are logged in with.
- **Related Tenant**: Filter plans based on the the [tenant](administering/tenant-management/tenants.md) your are logged in with.
- **Related Description Templates**: Filter descriptions based on the [templates](using/templates.md) their [descriptions](using/descriptions/index.md) are using.
- **Role**: Filter descriptions based on the role the user has on them.
- **Tags**: Filter descriptions based on the tags.
- **Reference Types**: Filter descriptions based on their [references](using/references.md). You can filter using more than one references by clicking the `Add a Reference Type` button and selecting your reference.
